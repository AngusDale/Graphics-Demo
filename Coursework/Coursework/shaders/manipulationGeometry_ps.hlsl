...
float4 calculateSpecular(float3 eyeVec, float3 lightVec, float3 normal, int index)
{
    ...
    float NdotH = max(abs(dot(halfway, normal)), 0.0f);
...
{    
    float intensity = saturate(abs(dot(normal, lightDirection)));
    return intensity;
}
...
float4 calculateSpotlight(float3 lightVec, float3 norm, float3 eyeVec, int index)
{    
    float NdotL = saturate(abs(dot(lightVec, norm)));
...
float4 main(InputType input) : SV_TARGET
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        switch (type[i])
        {
            ...
            case 1:
                ...
                {
                    // Has depth map data
                    float shadowFactor = isInShadow(depthMapTexture[i][0], pTexCoord[i][0], input.lightViewPos[i][0], bias[i]);
                    if (shadowFactor)
                    {
            ...
            case 2:
                if (hasDepthData(pTexCoord[i][0]))
                {
                    ...
                    if (shadowFactor)
                    {
                        lightColour += shadowFactor * atten * calculateSpotlight(lightVec, normalize(input.normal), normalize(input.viewVector), i);
                    }
                }
    ...
    return saturate(input.color * lightColour);
}